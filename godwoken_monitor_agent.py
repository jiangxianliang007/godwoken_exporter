...
from prometheus_client.core import CollectorRegistry, Gauge, Enum, Info
...
class RpcGet(object):
    ...
    def get_BlockDetail(self, block_hash):
        headers = {"Content-Type":  "application/json"}
        data = '{"id":2, "jsonrpc":"2.0", "method":"gw_get_block", "params":["%s"]}' %(block_hash)
        try:
            r = requests.post(
                url="%s" %(self.web3_url),
                data=data,
    ...
    def get_block_hash(self, blocknumber):
        headers = {"Content-Type":  "application/json"}
        data = '{"id":2, "jsonrpc":"2.0", "method":"gw_get_block_hash", "params":["%s"]}' %(blocknumber)
        try:
            r = requests.post(
                url="%s" %(self.web3_url),
                data=data,
...
# flask object
@NODE_FLASK.route("/metrics/godwoken")
def exporter():
    ...
    if "-1" in LastBlockDetail.values():
        ...
        PreviousBlock_hash = get_result.get_block_hash(hex((LastBlockDetail["blocknumber"]) - 1))
        PreviousBlockDetail = get_result.get_BlockDetail(PreviousBlock_hash["blocknumber_hash"])
